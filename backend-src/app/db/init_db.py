...
def init_db():
    ...
    try:
        ...
        # 简易迁移：为 course_content 表添加 scenes 列（OpenMAIC 式场景序列，若不存在）
        if 'course_content' in tables:
            columns = [col['name'] for col in inspector.get_columns('course_content')]
            if 'scenes' not in columns:
                with engine.connect() as conn:
                    conn.execute(text("ALTER TABLE course_content ADD COLUMN scenes JSON"))
                    conn.commit()
                print("迁移: 已为 course_content 添加列 scenes")
